...
async function getTopRatedMovies() {
  ...
  const commentIds = additionalDetails.map((detail) =>
    detail.commentIds.split(',')
  );
  ...
  const tRatedMovies = getMovieInfo.map(async (movie) => {
    const movieDetails = additionalDetails.filter(
      (detail) => detail.movieidx === movie.movieidx
    );
    const topDetail = movieDetails.reduce(
      (prev, current) => (prev.maxRate > current.maxRate ? prev : current),
      {}
    );
    const descriptions = descriptionDetails.filter((desc) =>
      topDetail.commentIds.includes(desc.commentid)
    );
    const description =
      descriptions.length > 0 ? descriptions[0].description : '';

    const commentids = descriptionDetails.filter((desc) =>
      topDetail.commentIds.includes(desc.commentid)
    );
    ...
    lowerRatedMovies.map(async (movie) => {
      const additionalDetails = await Comment.findAll({
        attributes: [
          ...
          [Sequelize.fn('MAX', Sequelize.col('rate')), 'maxRate'],
          [
            Sequelize.fn('GROUP_CONCAT', Sequelize.col('commentid')),
            'commentIds',
          ],
          [
            Sequelize.literal(
